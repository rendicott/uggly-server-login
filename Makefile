...
packageNameWindows := $(projectName)-windows-amd65-$(version).tar.gz
...
test:
	go build -o ./$(build_dir_windows)/$(projectName).exe -ldflags "-X main.version=$(version)"
	docker build . -t runtime-$(projectName)
	docker run -p 127.0.0.1:8889:80 -it runtime-$(projectName)
